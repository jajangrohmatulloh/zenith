...
export const CalendarView = () => {
    ...
    return (
        <div className="flex flex-col gap-4 bg-white/40 dark:bg-slate-900/80 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl shadow-indigo-500/5">
            <div className="flex flex-wrap items-center justify-between gap-y-2 gap-x-1 mb-4">
                <div className="flex items-center gap-1.5">
                    <button
                        onClick={() => setSelectedDate(null)}
                        className="p-1.5 -ml-1 rounded-lg text-indigo-500 dark:text-indigo-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                        title="Reset Filter"
                    >
                    ...
                    <div className="flex items-center bg-slate-100/50 dark:bg-slate-800/50 rounded-xl p-0.5 border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-sm">
                        <select
                            ...
                            className="appearance-none bg-transparent px-2 py-0.5 text-xs font-bold text-slate-900 dark:text-slate-100 transition-all cursor-pointer focus:outline-none"
                        >
                            {monthNames.map((name, i) => (
                                <option key={name} value={i} className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">{name}</option>
                            ))}
                        </select>
                ...
                <div className="flex items-center gap-1.5">
                    <Button
                        ...
                        size="sm"
                        onClick={goToToday}
                        className="flex items-center gap-1.5 px-2 py-1 h-7 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 border-none shrink-0"
                    >
                        <CircleDot className="w-3 h-3" />
                        <span className="text-[10px] font-bold uppercase tracking-wider">Today</span>
                    </Button>
                    <div className="flex items-center gap-0.5">
                        <Button variant="secondary" size="icon" onClick={prevMonth} className="h-7 w-7 rounded-lg">
                            <ChevronLeft className="w-3.5 h-3.5" />
                        </Button>
                        <Button variant="secondary" size="icon" onClick={nextMonth} className="h-7 w-7 rounded-lg">
